...
class ArraySpec extends AnyFlatSpec with Matchers with OptionValues {

  "2sum" should "work" in {
    def twoSum(nums: Array[Int], target: Int): Array[Int] = {

      val indices = nums.zipWithIndex.groupBy(_._1).map { case (num, indices) => (num, indices.map(_._2).toSet) }

      (for {
        index <- 0 to nums.length - 1
        otherIndex <- indices.get(target - nums(index)).getOrElse(Set.empty) if index != otherIndex
      } yield Array(index, otherIndex)).head
  ...
  "Set matrix zeroes" should "work" in {
    ...
    val input1 = Array(Array(1, 1, 1), Array(1, 0, 1), Array(1, 1, 1))
    val output1 = Array(Array(1, 0, 1), Array(0, 0, 0), Array(1, 0, 1))

    setZeroes(input1)

    input1 shouldBe output1

  }

  "Remove duplicates in ordered array" should "be done in O(n) time and O(1) space" in {

    def removeDuplicates2(nums: Array[Int]): Int = {

      var len = 0
      var lastElement: Int = Int.MinValue
      (0 to nums.length - 1).foreach { index =>
    ...
    def removeDuplicates(nums: Array[Int]): Int = {
      ...
      def helper(index: Int = 0, len: Int = 0, lastElement: Int = Int.MinValue): Int = {
        if (index > nums.length - 1) len
        else if (nums(index) != lastElement) {
          nums(len) = nums(index)
          helper(index + 1, len + 1, nums(index))
        } else
          helper(index + 1, len, nums(index))
      }

      helper()
    }

    removeDuplicates(Array[Int]()) shouldBe 0
    removeDuplicates(Array(1, 2, 3, 4)) shouldBe 4
    removeDuplicates(Array(1, 2, 2, 3, 4, 4)) shouldBe 4
  }

  "Best time to buy and sell stock" should "work" in {
    def maxProfit2(prices: Array[Int]): Int = {

      val len = prices.length
      var profit = 0
      var hasStock = false
      (0 to len - 2).foreach { index =>
        val currentPrice = prices(index)
        if (currentPrice <= prices(index + 1)) {
          if (!hasStock) {
            hasStock = true
            profit -= currentPrice
          }
        } else { // if prices will fall
          if (hasStock) {
            profit += currentPrice // sell it
            hasStock = false
          }
        }
      }

      // the last day sell it
      profit + (if (hasStock) prices(len - 1) else 0)
    }

    def maxProfit(prices: Array[Int]): Int = {
      def helper(index: Int = 0, profit: Int = 0, hasStock: Boolean = false): Int =
        if (index >= prices.length - 1)
          profit + (if (hasStock) prices.last else 0)
        else {
          val currentPrice = prices(index)
          if (currentPrice <= prices(index + 1))
            if (!hasStock)
              helper(index + 1, profit - currentPrice, true)
            else
              helper(index + 1, profit, hasStock)
          else if (hasStock)
            helper(index + 1, profit + currentPrice, false)
          else
            helper(index + 1, profit, hasStock)
        }
    ...
    maxProfit(Array.empty[Int]) shouldBe 0
    maxProfit(Array(7, 1, 5, 3, 6, 4)) shouldBe 7
    maxProfit(Array(1, 2, 3, 4, 5)) shouldBe 4
    maxProfit(Array(7, 6, 4, 3, 1)) shouldBe 0
  }

  "Rotate array" should "work with O(1) extra space" in {
    def rotate(nums: Array[Int], k: Int): Unit = {

      val temp = new Array[Int](nums.length)
      (0 to nums.length - 1).foreach { index =>
        val newPosition = (index + k) % nums.length
        temp(newPosition) = nums(index)
      }
    ...
    val a1 = Array(1, 2, 3)
    rotate(a1, 1)
    a1 shouldBe Array(3, 1, 2)

    val a2 = Array(1, 2, 3)
    rotate(a2, 0)
    a2 shouldBe Array(1, 2, 3)

    val a3 = Array.empty[Int]
    rotate(a3, 3)
    a3 shouldBe Array.empty[Int]
  }

  "Contains duplicate" should "work" in {
    ...
    containsDuplicate(Array.empty[Int]) shouldBe false
    containsDuplicate(Array(1)) shouldBe false
    containsDuplicate(Array(1, 2)) shouldBe false
    containsDuplicate(Array(1, 2, 1)) shouldBe true
    containsDuplicate(Array(1, 2, 1, 3)) shouldBe true
  }

  "Single number" should "work" in {

    def singleNumber(nums: Array[Int]): Int = {

      val map = scala.collection.mutable.Map.empty[Int, Int]
      nums.foreach { num =>
        map.put(num, map.get(num).getOrElse(0) + 1)
      }
      map.collect { case (num, occ) if occ == 1 => num }.headOption.get
    }

    singleNumber(Array(1)) shouldBe 1
    singleNumber(Array(1, 2, 2)) shouldBe 1
  }


  // Given two integer arrays nums1 and nums2, return an array of their intersection.
  //Each element in the result must appear as many times as it shows in both arrays and you may return the result in any order.
  "Intersection of Two Arrays II" should "work" in {

    def intersect(nums1: Array[Int], nums2: Array[Int]): Array[Int] = {

      def toMap(nums: Array[Int]): Map[Int, Int] = nums.groupBy(identity).map { case (num, ids) => num -> ids.size }

      val m1 = toMap(nums1)
      val m2 = toMap(nums2)

      m1.toArray.flatMap { case (num, freq1) =>
        val freq2 = m2.getOrElse(num, 0)
        val freq = Math.min(freq1, freq2)
        val arr = new Array[Int](freq)
        (0 to freq - 1).foreach { i =>
          arr(i) = num
        }
        arr
      }
    ...
    intersect(Array(1, 2, 2, 1), Array(2, 2)) shouldBe Array(2, 2)
    intersect(Array(4, 9, 5), Array(9, 4, 9, 8, 4)) shouldBe Array(4, 9)
  ...
  "Plus one" should "work" in {
    def plusOne(digits: Array[Int]): Array[Int] = {

      val len = digits.size

      def helper(index: Int, remainder: Int = 0): Int = {
        if (index == -1) remainder
        else {
          val digit = digits(index)
          val digitPlus1 = digit + remainder
          val (newDigit, newRemainder) = if (digitPlus1 == 10) (0, 1) else (digitPlus1, 0)
          digits(index) = newDigit
          helper(index - 1, newRemainder)
        }
      }

      val finalRemainder = helper(len - 1, 1)
  ...
  //  Input: nums = [0]
  //  Output: [0]
  "Move zeroes" should "work" in {

    def moveZeroes2(nums: Array[Int]): Unit = {
      def helper(endIndex: Int, currentIndex: Int): Unit = {
        //        println(s"current $currentIndex, end $endIndex, array ${nums.mkString(",")}")
        if (currentIndex > -1) {
          if (nums(currentIndex) == 0) {
            (currentIndex + 1 to endIndex).map { i =>
              nums(i - 1) = nums(i)
            }
            nums(endIndex) = 0
            helper(endIndex - 1, currentIndex - 1)
          }
          helper(endIndex, currentIndex - 1)
        }
      }

      helper(nums.length - 1, nums.length - 1)

    }

    def moveZeroes(nums: Array[Int]): Unit = {
      var zeroCount = 0
      (0 to nums.size - 1).foreach { index =>
        val num = nums(index)
        if (num == 0) {
          zeroCount += 1
        } else {
      ...
      val len = nums.length
      (0 to zeroCount - 1).foreach { index =>
        nums(len - 1 - index) = 0
      }

    }

    val nums1 = Array(0, 1, 0, 3, 12)
    moveZeroes(nums1)
    nums1 shouldBe Array(1, 3, 12, 0, 0)

    val nums2 = Array(0)
    moveZeroes(nums2)
    nums2 shouldBe Array(0)
  }

  "Valid Sodoku" should "work" in {

    def isValidSudoku(board: Array[Array[Char]]): Boolean = {

      def column(colIndex: Int, br: Array[Array[Char]]): Array[Char] = (0 to br.length - 1).map { rowIndex =>
        br(rowIndex)(colIndex)
      }.toArray

      def is9Valid(cells: Array[Char]): Boolean = {
        println(s"is9Valid ${cells.toList}")
        val nums = cells.filter(_ != '.')
        nums.size == nums.distinct.size
      }

      def littleBoard(startRowIndex: Int, startColIndex: Int): Array[Array[Char]] = {
        val result: Array[Array[Char]] = Array.ofDim[Char](3, 3)
        (0 to 2).foreach { rowIndex =>
          (0 to 2).foreach { colIndex =>
            result(rowIndex)(colIndex) = board(startRowIndex + rowIndex)(startColIndex + colIndex)
          }
        }
        result
      }

      def isBoardValid(br: Array[Array[Char]]): Boolean = {
        br.forall { row => is9Valid(row) } &&
          (0 to br.length - 1).forall { colIndex => is9Valid(column(colIndex, br)) }
      }

      def isLittleBoardValid(br: Array[Array[Char]]): Boolean =
        is9Valid(br.flatten)

      if (board.length == 3)
        isLittleBoardValid(board)
      else
        isBoardValid(board) && {
          board.length <= 3 || {
            val offsets: Iterable[(Int, Int)] = for {
              rowOffset <- Iterable(0, 3, 6) if rowOffset < board.length
              colOffset <- Iterable(0, 3, 6) if colOffset < board.length
            } yield (rowOffset, colOffset)

            offsets.forall { case (rowOffset, colOffset) =>
              isLittleBoardValid(littleBoard(rowOffset, colOffset))
            }
          }
        }
    }
    ...
    val input1: Array[Array[Char]] = Array(
      Array('.', '1', '.'),
      Array('.', '.', '.'),
      Array('.', '.', '1'))
    ...
    isValidSudoku(input1) shouldBe false
    isValidSudoku(input2) shouldBe true
    isValidSudoku(input3) shouldBe false
  }

  "Rotate matrix in-place" should "work" in {
...
//    val input1 = Array(Array(1, 2), Array(3, 4))
//    rotate(input1)
//    input1 shouldBe Array(Array(3, 1), Array(4, 2))

    val input2 = Array(Array(7,2,1),Array(4,5,6),Array(9,8,3))
    rotate(input2)
    input2 shouldBe Array(Array(7,4,1),Array(8,5,2),Array(9,6,3))
  }

  "Finding Maximum Subarray" should "work" in {
    def maxSubArray(nums: Array[Int]): Int = {

      val lastIndex = nums.size - 1

      def findEndIdx(idx: Int = 0, maxEndIdx: Int = 0, maxEndSum: Int = Integer.MIN_VALUE, sum: Int = 0): Int =
        if (idx > lastIndex) maxEndIdx
        else {
          val newSum = sum + nums(idx)
          val (newMaxEndIdx, newMaxEndSum) =
            if (newSum > maxEndSum) (idx, newSum)
            else (maxEndIdx, maxEndSum)

          findEndIdx(idx = idx + 1, maxEndIdx = newMaxEndIdx, maxEndSum = newMaxEndSum, sum = newSum)
        }

      val maxEndIdx = findEndIdx()


      var maxStartIdx = maxEndIdx
      var maxStartSum = nums(maxStartIdx)
      var sum = 0
      (maxEndIdx to 0 by -1).foreach { idx =>
        sum += nums(idx)
        if (sum > maxStartSum) {
          maxStartIdx = idx
          maxStartSum = sum
...
//    maxSubArray(Array(-2,1,-3,4,-1,2,1,-5,4)) shouldBe 6
//    maxSubArray(Array(-2, 1)) shouldBe 1
